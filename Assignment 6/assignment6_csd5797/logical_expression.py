...
def get_symbols(expression):
    ...
    if expression.symbol[0]: # If base case (symbol)
        symbols.append(expression.symbol[0])

    else: # else subexpression
        for subexpression in expression.subexpressions:
            for symbol in get_symbols(subexpression):
...
def PL_True(statement,model):
    if statement.symbol[0]:
        ...
        for exp in statement.subexpressions:
            result = result and PL_True(exp,model)
        return result

    elif statement.connective[0].lower() == 'or':
        result = False
        for exp in statement.subexpressions:
            result = result or PL_True(exp,model)
        return result

    elif statement.connective[0].lower() == 'xor':
        result = False
        for exp in statement.subexpressions:
            isExpTrue = PL_True(exp,model)
        ...
        isLeftTrue = PL_True(left,model)
        isRightTrue = PL_True(right,model)
...
def check_true_false(knowledge_base, statement):
    ...
    symbols = get_symbols(knowledge_base)
    for symbol in get_symbols(statement):
        symbols.append(symbol)

    # remove symbols from model that are already present in the 
    for symbol in model:
        if symbol in symbols:
            symbols.remove(symbol)

    truthOfStatement = TT_Check_All(knowledge_base, statement, symbols, model)
    ...
    truthOfNegation = TT_Check_All(knowledge_base, negation, symbols, model)
...
def TT_Check_All(KB, statement, symbols, model):
    if not symbols:
        # print "truth of statement is : %s" % PL_True(statement,model)
        if PL_True(KB,model):
            # print "KB is true"
        ...
        p = symbols.pop(0)
        return TT_Check_All(KB, statement, symbols, extend(model,p,True)) and TT_Check_All(KB, statement, symbols, extend(model,p,False))
